...
void DisconnectBlockingDrivers() {
    ...
    EFI_STATUS Status = uefi_call_wrapper(BS->LocateHandleBuffer, 5, ByProtocol, &gEfiDiskIoProtocolGuid, NULL, &HandleCount, &Handles);
    ...
    for (uint64_t Index = 0; Index < HandleCount; Index++) {
        ...
        wchar_t* DevicePathString = ConvertDevicePathToText(DevicePathFromHandle(Handles[Index]));
