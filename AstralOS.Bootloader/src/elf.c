...
bool ValidateEhdr(Elf64_Ehdr* hdr) {
    ...
    if (hdr->e_type != ET_EXEC && hdr->e_type != ET_DYN) {
        Print(L"ELF: not ET_EXEC/ET_DYN (unsupported)");
        return false;
    }
...
uint64_t LoadElf(Elf64_Ehdr* hdr) {
    ...
    // Allocate enough pages for total_size + max_align
    UINTN pages = EFI_SIZE_TO_PAGES(total_size + max_align);
    status = uefi_call_wrapper(BS->AllocatePages, 4, AllocateAnyPages, EfiLoaderData, pages, &raw_addr);
    if (EFI_ERROR(status)) {
        return 0x0; // allocation failed
    }

    // Align the base pointer
    uint8_t* raw = (uint8_t*)(UINTN)raw_addr;
    ...
    if (dynamic) {
        ...
        */
        ...
        apply_relocations(base, rela, relasz, relaent, symtab, strtab, syment, NULL);
    }
        ...
        switch (type) {
            ...
            case R_X86_64_PC32:
                *reloc_addr = sym_vaddr + rela->r_addend - (uint64_t)reloc_addr;
                break;
            default:
                Print(L"Unknown relocation type!");
                break;
        }
    }
}